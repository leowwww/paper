...
df_stock = pd.read_excel('result.xlsx',usecols=[0 , 1,2])
df_stock_new = df_stock
#df_stock_new.set_index(['Open' , "Close"])
#df_stock_new['y'] = df_stock_new.iloc[:,1]
x=np.array(df_stock_new.iloc[:,[0,1]].values)
y=np.array(df_stock_new.iloc[:,2].values)
#y=np.array(y)
...
#(92808,)
x_seq_4d = x_seq.reshape(18560,1,7,2)
...
for opt in optimizers:
    for act in activation_functions:
        ...
        model = Sequential()
        model.add(Conv2D(64, kernel_size=(3,3), strides=(1, 1), padding='same',activation=act,input_shape=(1,7,2))) #可以改成(1,7,2)
